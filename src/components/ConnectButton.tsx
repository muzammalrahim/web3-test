...
export default function ConnectButton() {
 ...
 const sendBaby = useCallback(async () => {
  const web3 = new Web3(library.provider);
  const ctx = new web3.eth.Contract(abi as AbiItem[], "0xc748673057861a797275CD8A068AbB95A902e8de");
 ...
 function fromWei(web3: { utils: { fromWei: (arg0: any) => any } }, val: { toString: () => any }) {
  if (val) {
   val = val.toString();
 ...
 const valueload = useCallback(async () => {
  ...
  const ctx = new web3.eth.Contract(abi as AbiItem[], "0xc748673057861a797275CD8A068AbB95A902e8de");
  //   Test Code
  const check = await ctx?.methods?.balanceOf(account).call();
  console.log(check);
  /////////////////////
  //   const ctx = new web3.eth.Contract(abi as AbiItem[], "0x4cdc7635c477017dFc1A9F4A52E3a36f2999eBd3");
  if (account) {
   const balance = await web3.eth.getBalance(account);
   setBalance(Number(fromWei(web3, balance)).toFixed(5));
   const gasPrice = await web3.eth.getGasPrice();
   setGasFee(gasPrice);

   //    const value1 = await ctx.methods.balanceOf(account).call({ gasPrice: Number(gasPrice) * 100 });
   //    const babyBalance = await ctx.methods
   //     .balanceOf(account)
   //     .call({ from: account, gas: Number(gasPrice) * 100 });
  ...
  <>
   ...
   {account ? (
    ...
    >
     ...
     <Box display="block" justifyContent="space-between" alignItems="center" mb="4">
      <Text color="#158DE8" fontWeight="medium">
       Send {mode}:
      </Text>
      <Input bg="#EBEBEB" size="lg" value={recieverAdd} onChange={handleChangeAddress} />
     </Box>
     <Box display="flex" alignItems="center" mb="4">
      <Input bg="#EBEBEB" size="lg" value={sendAmount} onChange={handleChangeAmount} />
      <Button
